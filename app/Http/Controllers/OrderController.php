...
class OrderController extends Controller
{
    ...
    public function view_order($order_code){
        ...
        $order              =    Order          ::where('order_code'    ,   $order_code)        ->first();        // 1 row
        
        if(!$order)         
        
            return          \redirect('/manage-order')      ->with('message' , "order ko con ton tai");
        ...
        $order_details_product      =    OrderDetails::with('product')
                                            ...
                                            ->where('order_code'    ,   $order_code)    ->get();        // table
        
                                            
        foreach($order_details_product as $key => $order_d){
            
            $product_coupon     =    $order_d->product_coupon;      // coupon code
        }
        ...
        $order_details      =    OrderDetails   ::with('product')
                                                ...
                                                ->where('order_code'    ,   $order_code)        ->get();        // 1 table
	...
	//	    $now                =    Carbon::now('Asia/Ho_Chi_Minh')->toDateString();
		...
		// in canced order case  ,  restore the previous quantity
		
		else  /* ($order->order_status != 2   &&   $order->order_status != 3 ) */  {      //not yet treated
...
//		$order_details    = OrderDetails  ::where('order_code'    ,   $checkout_code)     ->get();
		...
		$order_details_product    =    OrderDetails   ::with('product')
		                                              ...
		                                              ->where('order_code'    ,    $checkout_code)    ->get();
